...
async function handleRole(message, args) {
    ...
    if (!member) {
        return message.reply('❌ Please mention a member.\nUsage: `,role @member RoleName`');
    }

    // Get role name (everything after the member mention)
    ...
    // Find role by name (case-insensitive) or by mention/ID
    ...
    if (!role) {
        return message.reply(`❌ Role "${roleName}" not found. Make sure the role name is correct.`);
    }

    // Check if bot can manage this role
    if (role.position >= message.guild.members.me.roles.highest.position) {
        return message.reply('❌ I cannot manage this role. It is higher than or equal to my highest role.');
    }

    // Check if user can manage this role
    if (role.position >= message.member.roles.highest.position) {
        return message.reply('❌ You cannot manage this role. It is higher than or equal to your highest role.');
    }

    try {
        if (member.roles.cache.has(role.id)) {
            await member.roles.remove(role);
            message.reply({ content: `✅ Removed **${role.name}** from ${member.user.tag}`, allowedMentions: { parse: [] } });
        } else {
            await member.roles.add(role);
            message.reply({ content: `✅ Added **${role.name}** to ${member.user.tag}`, allowedMentions: { parse: [] } });
        }
    } catch (error) {
        console.error('Error managing role:', error);
        message.reply(`❌ Failed to manage role: ${error.message}`);
    }
}
